...
try:
    ...
    base_phase_mat = io.loadmat('../128beam_2D_wider.mat')
    ...
    for beam_idx in range(0, codebook_size):
        ...
        data['args'] = {'sector_idx': beam_idx+16,
            'sector_type': 0,
            'rf_modules_vec': rf_modules_vec,
            'mag':  (''.join(base_phase_mat['beam_weight'][0][(beam_list[beam_idx])][0][0].tolist()).encode('utf-8','ignore')),
            'phase': (''.join(base_phase_mat['beam_weight'][0][(beam_list[beam_idx])][0][1].tolist()).encode('utf-8','ignore')),
            'amp': "66666666",
        }
